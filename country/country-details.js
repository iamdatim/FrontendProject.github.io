    ...
    const country = jsonData.find(country => country.alpha2Code === countryCode);
    ...
    if (country) {
      ...
      const capital = createParagraph("Capital", country.capital);
      const population = createParagraph("Population", country.population.toLocaleString());
      const region = createParagraph("Region", country.region);
    ...
    document.addEventListener("DOMContentLoaded", () => {
      const card = document.getElementById("card"); // Replace "card" with the ID or class of your card element

      card.addEventListener("click", () => {
        // Redirect to new page with country details
        window.location.href = `country-details.html?code=${countryCode}`; // Replace "country-details.html" with the actual URL of the country details page
      });
    });